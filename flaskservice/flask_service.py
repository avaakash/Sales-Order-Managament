...
def predict():
    if request.method == 'POST':
        ...
        model_features = joblib.load(r"Files/"+pickleFile)
        myClass = __str2Class(transformationFile,"Files")
        myClassInstance = myClass(
            map_cust_mean_delay=model_features['map_cust_mean_delay'], 
            sorted_cpt=model_features['sorted_cpt'],
            map_cust=model_features['map_cust'])

        predictions = myClassInstance.getPredictions(data,model_features['model'])
